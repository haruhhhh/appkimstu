...
def main():
    ...
    if uploaded_file is not None:
        ...
        # 推論の確率を小数点以下3桁で丸め×100(%に変換)
        pred0 = round(prediction[0],3) * 100  # 碇ゲンドウの確率(%)
        pred1 = round(prediction[1],3) * 100  # マダオの確率(%)
        # 推論で得られたクラス番号(初期値は0)によって出力結果を分岐
        if classNo == 0:
            st.subheader(f"これは{pred0}％の確率で「碇ゲンドウ」です！")
        else:
            st.subheader(f"これは{pred1}％の確率で「マダオ」です！")
